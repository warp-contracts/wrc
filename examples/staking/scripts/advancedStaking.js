...
async function withdraw(staking, amount) {
  let interaction = await staking.writeInteraction({ function: 'withdraw', amount }, { strict: true });
...
async function approveAndStake() {
  ...
  const erc20 = warp
    .contract(getContractTxId(warp.environment, __dirname, 'erc20'))
    .setEvaluationOptions({ internalWrites: true })
    .connect(ownerWallet);

  const stakingTxId = getContractTxId(warp.environment, __dirname, 'staking');
  const staking = warp.contract(stakingTxId).setEvaluationOptions({ internalWrites: true }).connect(ownerWallet);
